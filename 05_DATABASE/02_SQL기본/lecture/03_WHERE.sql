...
select *
from employees
where gender = 'M'; -- 성별이 M인 데이터만

-- select에 조회칼럼으로 선택하지 않은 칼럼으로 조건을 걸 수 있음.
select first_name, last_name
from employees
where gender = 'M'; -- 성별이 M인 데이터만

-- Column A != B : 칼럼의 내용중 B와 같지 않은 데이터만 보여주기 -> 확장문법
-- Column A <> B : 칼럼의 내용중 B와 같지 않은 데이터만 보여주기 -> 표준
select first_name, last_name, gender
from employees
# where gender != 'M'; -- 성별이 M이 아닌 데이터만
where gender <> 'M'; -- 성별이 M이 아닌 데이터만
...
select
    first_name
from employees
where first_name like 'Aa%'; -- first_name이 Aa 시작하는 모든 데이터

select
    first_name
from employees
where first_name not like 'Aa__d%';-- first_name이 Aa 시작하고 d로 끝나는 5글자인 모든 데이터
