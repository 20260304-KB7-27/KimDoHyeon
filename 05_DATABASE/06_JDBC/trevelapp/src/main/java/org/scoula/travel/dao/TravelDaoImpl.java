...
public class TravelDaoImpl implements TravelDao{
    ...
    public List<String> getDistricts() {
        List<String> districts = new ArrayList<>(); // 결과 담아줄 List
    ...
    public List<TravelVO> getTravels() {
        ...
        List<TravelVO> travels = new ArrayList<>(); // 결과 담아줄 List
    ...
    private TravelVO map(ResultSet rs) throws SQLException {
        /*
        * Builder Pattern
        * - 단계적을 ㅗ값을 설정하고 객체를 생성하는 패턴
        * - 필드가 많거나 선택적으로 입력해야하는 값이 많은 객체를 만들 때 사용
        * */
    ...
    public List<TravelVO> getTravels(int page) {
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            pstmt.setInt(2, start + count); // 몇개를 가지고 올지
    ...
    public Optional<TravelVO> getTravel(Long no) {
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(sql);
            ...
            try(ResultSet rs = pstmt.executeQuery()) {
                // 조회한 데이터각 없으면 Optional.empty(); 반환
