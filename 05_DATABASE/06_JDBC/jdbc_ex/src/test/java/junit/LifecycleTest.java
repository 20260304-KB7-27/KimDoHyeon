...
public class LifecycleTest {
    ...
    // DB 연결 같은 전체테스트에서 공통으로 필요한 자원들을 초기화할 때 (static 필수)
    @BeforeAll
    static void beforeAll() {
        System.out.println("beforeAll : 모든 테스트 전 최초 1회 실행");
    }

    @AfterAll // 사용한 자원을 반환하거나 @BeforeAll에서 열어둔 자원 정리ㄴ
    static void afterAll() {
        System.out.println("afterAll : 모든 테스트 실행 후 최초 1회 실행");
    }

    @BeforeEach // 테스트 마다 독립적인 초기상태 보장이 필요할 때
    void beforeEach() {
        System.out.println("beforeEach: 각 테스트 전 실행");
    }

    @AfterEach // 테스트가 남긴 잔여(mock 데이터 변경 등) 제거
    void afterEach() {
